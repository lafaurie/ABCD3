...
public class LibraryAddArea extends BaseEditableArea {
	...
	public LibraryAddArea(ViewController controller, CRUDTreeTable tableLibrary) {
		super();
	...
	public Composite createUI(Composite parent, IGridViewEntity entity,
		...
		saveLibraryFragment = new RegisterLibraryFragment(library, dimension, 1);
		...
		saveButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (saveLibraryFragment.getValidator().decorationFactory
					...
					if (saveLibraryFragment.getValidator().decorationFactory.AllControlDecorationsHide()) {
						String libraryName = saveLibraryFragment
								.getNameLibraryText().getText().replaceAll(" +", " ").trim();
						Library existLibraryWithThisName = ((LibraryViewController) controller)
								.findLibraryByName(libraryName);
						
						if (existLibraryWithThisName == null) {
							...
							if (existLibraryWithThisDefHome == null ) {
								...
								library.setEnabled(isisHome!="");
								Library librarySaved = ((LibraryViewController) controller)
										.saveLibrary(library);
								library = null;
								manager.add(new BaseGridViewEntity<Library>(
										librarySaved));
								manager.refresh();
								Composite viewSmg = ((LibraryViewArea) tableLibrary
										.getActiveArea()).getMsg();
