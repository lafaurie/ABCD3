...
public class ReportUpdateEditableArea extends BaseEditableArea {
	...
	public ReportUpdateEditableArea(ViewController controller) {
		super();
	...
	public Composite createUI(final Composite parent, IGridViewEntity entity, IVisualEntityManager arg2) {
		reportEntity = (Report) entity.getRow();
		indicatorList.clear();
		indicatorList.addAll(reportEntity.getIndicators());
		...
		textlabelNameReport.setText(reportEntity.getNameReport());
	...
	private void createTables(final Composite parent) {
		...
		tablaIndicator.addListener(SWT.Resize, new Listener() {
			...
			private static final long serialVersionUID = -3481688390243676047L;

			@Override
			public void handleEvent(Event arg0) {
				ReportUpdateEditableArea.this.refresh();
			...
			public void widgetSelected(SelectionEvent e) {
				if (validator.decorationFactory.isRequiredControlDecorationIsVisible()) {
					...
					if (!(indicatorList.isEmpty())) {
						...
						Report report = ((AllManagementController) controller).getReport().viewReport(reportEntity.getId());
						...
						if (exist == null || exist.getId().equals(reportEntity.getId())) {
							Report auxReport = ((AllManagementController) controller).getReport().addReport(report);
							manager.save(new BaseGridViewEntity<Report>(auxReport));
							manager.refresh();
						} else {
	...
	public void l10n() {
		...
		headerLabel.setText(MessageUtil.unescape(AbosMessages.get().LABEL_EDIT_REPORT));
