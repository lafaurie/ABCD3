...
public class EventKeyRecordQuery implements KeyListener{
	...
	public EventKeyRecordQuery(ProxyController controller, String dataBaseName,
			Composite view, QueryStructure queryStructure){
	...
	private List<Record> buildQuery(){
		...
		if(fieldsWithValues.size() == 1){
			...
			try {
				...
				records = exemplaryRecord.findByOptionsExemplaryRecords(fieldsWithValues, dataBaseName, dataBaseManager,
						isisDefHome);
