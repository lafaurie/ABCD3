...
public class EventDown implements Listener {
	...
	public EventDown(FieldStructure fieldStructure, SubFieldStructure subFieldStructure, Composite parent, ExpandItem expandItem,
	...
	public void handleEvent(Event arg0) {
		...
		if (visualPosition != parent.getTabList().length - 1) {
			...
			int positionInsert = (positionStructure + 1 + contentChangeSize) - contentSize;
			...
			ToolItem up = ((SingleCataloguingComponent)(parent.getTabList()[visualPositionChange])).getUp();
			ToolItem down = ((SingleCataloguingComponent)(parent.getTabList()[visualPosition])).getDown();

			if(!up.isDisposed())
				up.setEnabled(true);

			if(visualPositionChange == parent.getTabList().length-1){
				if(!down.isDisposed())
			...
			for (int i = 0; i < parent.getTabList().length; i++) {
				...
				if (i == 0) {
					...
					ToolItem up2 = ((SingleCataloguingComponent)(parent.getTabList()[i])).getUp();
