...
public class NotPaginateTable extends Composite implements TreeTable {
	...
	public NotPaginateTable(Composite parent, int style) {
		super(parent, style);
		this.parent = parent;
		...
		this.visualEntityManager = new NotPaginateVisualEntityManager(this); 
	...
	public void createTable(TreeTableColumn[] configuration) {
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			TreeColumn treeColumn = this.createTreeColumn(columnData,true);
			String header = "";
					...
					while (!itemsQueue.isEmpty() && !handled) {
						...
						for (int j = firstActionColumnIndex; j < tree.getColumnCount(); j++) {
							...
							if (cellBounds.contains(pt)) {
								...
								eventData.source = NotPaginateTable.this;
		...
		this.addControlListener(new ControlListener() {
			...
			public void controlResized(ControlEvent event) {
				NotPaginateTable.this.controlResized(event);
			}
	...
	public void refresh() {
		if (!NotPaginateTable.this.tree.isDisposed()) {
			NotPaginateTable.this.getShell().layout(true, true);
			NotPaginateTable.this.getShell().redraw();
			NotPaginateTable.this.getShell().update();
		}
	}
	...
	final Runnable refreshRun = new Runnable() {
		...
		public void run() {
			if (!NotPaginateTable.this.tree.isDisposed()) {
				NotPaginateTable.this.tree.layout(true, true);
				NotPaginateTable.this.tree.redraw();
				NotPaginateTable.this.tree.update();
			}
		}
	};
	...
	public synchronized void destroyEditableArea() {
	...
	public synchronized void createEditableArea(IEditableArea area, IGridViewEntity entity, IVisualEntityManager manager) {
		if (editionVisible) {
			...
			if (area != null) {
				...
				if (area.closable()) {
					...
					if (area == watchArea) {
						...
						closeBtn.addSelectionListener(new SelectionListener() {
							...
							@Override
							public void widgetSelected(SelectionEvent event) {
								NotPaginateTable.this.editionComposite.dispose();
								editionVisible = false;
								NotPaginateTable.this.notifyListeners(SWT.Resize, new Event());
							}
						...
						closeBtn.addSelectionListener(new SelectionListener() {
							...
							public void widgetSelected(SelectionEvent event) {
								...
								MessageDialogUtil.openConfirm(Display.getCurrent().getActiveShell(), MessageUtil.unescape(AbosMessages.get().MESSAGE_QUESTION),
										MessageUtil.unescape(AbosMessages.get().MSG_WARN_CANCEL_OPERATION), new DialogCallback() {
				...
				this.editionComposite.addDisposeListener(new DisposeListener() {
					...
					public void widgetDisposed(DisposeEvent event) {
						NotPaginateTable.this.editionVisible = false;
						parent.pack(true);
	...
	private void forwardResizedEvent() {
		this.addListener(SWT.Resize, new Listener() {
			@Override
			public void handleEvent(Event event) {
				if (NotPaginateTable.this.getParent() != null) {
					NotPaginateTable.this.getParent().notifyListeners(SWT.Resize, event);
				}
			}
		});
	}
	...
	public void onPageChange() {
		...
		event.totalPages = 1;
		event.totalElements = Integer.MAX_VALUE;
		
		event.startIndex = 0;
		event.endIndex = Integer.MAX_VALUE;
		event.sortData = this.sortData;
		event.oldPage = 1;
		event.currentPage = 1;
		event.isFirst = true;
		event.isLast = true;
		event.pageSize = Integer.MAX_VALUE;
