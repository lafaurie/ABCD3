...
public class LabelHeaderPainter extends Painter{
	...
	protected Control paintLeftMiddleControl(Control control, Integer calculatePercent) {
		FormDatas.attach(control).atTopTo(helper.getTop(), 25).atLeftTo(helper.getLeft(), 15).withHeight(23).withWidth(SWT.DEFAULT).atLeft(15);
	...
	protected Control paintRightMiddleControl(Control control, Integer calculatePercent) {
		FormDatas.attach(control).atTopTo(helper.getTop(), 25).atLeftTo(helper.getLast(), 15).withHeight(23).withWidth(SWT.DEFAULT).atLeft(15);
		helper.setLast(control);
		return control;
	}
	...
	protected Control paintLeftControl(Control control, Integer percent) {
		FormDatas.attach(control).atTopTo(helper.getTop(), 25).withHeight(23).withWidth(SWT.DEFAULT).atLeft(15);
